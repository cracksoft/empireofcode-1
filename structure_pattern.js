...
function morph(x, pl) {
  if (pl === 2 || /[a-z]/.test(x)) return 1
  return /[A-Z]/.test(x)?2:3
}

function checkStructure(pattern, structure, patternLevel) {
  patternLevel = (typeof patternLevel === 'undefined' ? 2:patternLevel)
  let a = (Array(32).join("0") + structure.split("").map(x => /[0-9]/.test(x)?0:morph(x, patternLevel)).join("")).slice(-32)
  let b = (Array(32).join("0") + pattern.toString(patternLevel)).slice(-32)
